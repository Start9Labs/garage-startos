...
This repo packages [Garage](https://garagehq.deuxfleurs.fr/) for StartOS.
...
Garage runs the `dxflrs/garage` image pinned via `dockerTag` in `startos/manifest/index.ts`. The upstream project lives on `git.deuxfleurs.fr`, not GitHub — check for new releases at <https://git.deuxfleurs.fr/Deuxfleurs/garage/releases>.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `dxflrs/garage:v<new version>`.
